...
impl CodeGen {
    ...
    pub fn generate(&mut self, program: &Program) -> String {
        ...
        self.emit("declare i8* @cscript_malloc(i32)");
        self.emit("declare i32 @cscript_free(i8*)");
        self.emit("declare i32 @cscript_check_bounds(i8*, i32)");
        self.emit("");
        ...
        for (s, var_name) in &self.string_constants {
            ...
            header.push_str(
                &format!(
                    "{} = private unnamed_addr constant [{}_x_i8] c\"{}\\00\", align 1\n",
                    var_name, len, escaped
                )
                .replace("_x_", " x "),
            );
        }
    ...
    fn gen_function(
        ...
        let param_str = params
            .iter()
            .map(|(t, _)| self.get_llvm_type(t))
            .collect::<Vec<_>>()
            .join(", ");

        self.emit(&format!(
            "define {} @{}({}) {{",
            llvm_ret_type, name, param_str
        ));
        self.emit("entry:");
        ...
        for (i, (ty, param_name)) in params.iter().enumerate() {
            ...
            self.symbol_table
                .insert(param_name.clone(), (alloca.clone(), ty.clone()));

            self.emit(&format!(
                "  store {} %{}, {}* {}, align 4",
                llvm_ty, i, llvm_ty, alloca
            ));
        }
    ...
    fn gen_statement(&mut self, stmt: &Statement) {
        match stmt {
            Statement::VarDecl(ty, name, init) => {
                ...
                self.symbol_table
                    .insert(name.clone(), (ptr.clone(), ty.clone()));
            ...
            Statement::Assign(lhs, rhs) => {
                ...
                let (ptr, ptr_ty) = match lhs {
                    ...
                    Expression::ArrayAccess(name, index) => {
                        let (array_ptr, array_type) =
                            self.symbol_table.get(name).expect("Unknown array").clone();
                        ...
                        self.emit(&format!(
                            ...
                            elem_ptr, elem_type, elem_type, array_ptr, idx_val
                        ));
                        (elem_ptr, format!("{}*", elem_type))
                    }
                    Expression::UnaryOp(UnaryOpType::Deref, target) => {
                        // *target = val
                        // Evaluate target to get the address
                        let (addr, addr_ty) = self.gen_expression(target);
                        // addr is the pointer to store to. addr_ty is like i32*
                        (addr, addr_ty)
                    }
            ...
            Statement::If(cond, then_block, else_block) => {
                ...
                let target_else = if else_block.is_some() {
                    &label_else
                } else {
                    &label_merge
                };
            ...
            Statement::ArrayDecl(ty, name, size) => {
                ...
                self.symbol_table.insert(
                    name.clone(),
                    (ptr.clone(), Type::Pointer(Box::new(ty.clone()))),
                );
            }
        }
    }
    ...
    fn gen_expression(&mut self, expr: &Expression) -> (String, String) {
        match expr {
            ...
            Expression::FuncCall(name, args) => {
                ...
                let ret_ty = if name == "cscript_fopen"
                    || name == "cscript_fwrite"
                    || name == "cscript_fclose"
                    || name == "cscript_system"
                {
                    ...
                    self.emit(&format!(
                        "  {} = call i8* @cscript_malloc({})",
                        res, arg_str
                    ));
                    return (res, "i8*".to_string());
                } else if name == "free" {
                    // free returns an int status code
                    self.emit(&format!("  {} = call i32 @cscript_free({})", res, arg_str));
                    return (res, "i32".to_string());
                } else if name == "print" {
                ...
                match op {
                    UnaryOpType::AddressOf => {
                        ...
                        if let Expression::Identifier(name) = &**expr {
                            if let Some((ptr, var_type)) = self.symbol_table.get(name) {
                                let llvm_ty = self.get_llvm_type(var_type);
                                // Return the alloca pointer directly
                                (ptr.clone(), format!("{}*", llvm_ty))
                            } else {
                                panic!("Unknown variable: {}", name);
                            }
                        } else {
                    ...
                    UnaryOpType::Deref => {
                        // Handle *x
                        let (val, val_ty) = self.gen_expression(expr);
                        // val is the address. val_ty is like i32*
                        // We want to load from it.
                        // Result type is the element type of val_ty.
                        let elem_ty = if val_ty.ends_with('*') {
                            &val_ty[..val_ty.len() - 1]
                        } else {
                            panic!("Dereferencing non-pointer type: {}", val_ty);
                        };

                        // Check bounds if it's a pointer (heuristic: if it's i8* or similar)
                        // For now, we don't have enough info to know if it's a malloc'd pointer or stack pointer easily
                        // without more complex tracking.
                        // But we can add the check_bounds call if we want to be safe.
                        // However, check_bounds takes i8*, so we'd need to cast.

                        let res = self.new_temp();
                        self.emit(&format!(
                            "  {} = load {}, {} {}, align 4",
                            res, elem_ty, val_ty, val
                        ));
                        (res, elem_ty.to_string())
                    }
                }
